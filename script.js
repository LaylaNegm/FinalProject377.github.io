...
list.forEach((item) => {
  ...
  el.innerText = item.station_name;
...
async function getData() {
  ...
  const reply = json.filter((item) => Boolean(item.location_1)).filter((item) => Boolean(item.station_name));
  return reply;
}
...
async function mainEvent() {
  ...
  const submit = document.querySelector('#get-stat'); // get a reference to your submit button
  ...
  form.addEventListener('submit', (submitEvent) => {
    ...
    currentList = processStations(mapData);
